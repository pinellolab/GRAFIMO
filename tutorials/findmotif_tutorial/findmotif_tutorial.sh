...
# on genome variation graphs (VGs) by using GRAFIMO.
#
# This example searches a toy motif called 'example.meme' 
...
#
# Note 2 haplotypes are encoded in your VGs
grafimo findmotif -d data/mygenome/ -m $motif -b $bedfile

# The results are stored in the directory 'grafimo_out_PID_MOTIFID'
# 
# A TSV report, an HTML report and a GFF3 report are contained in 
# the directory. The GFF3 can be loaded on UCSC genome browser as 
# custom track.



# GRAFIMO computes a log-likelihood score, a P-value and a q-value
# for each retrieved potential motif occurrence. Suche measures are 
...
# that the background distribution is the one described
# in 'bg_nt' file and we want store the results in 'grafimo_out_bg'.
# We use `-k` and `-o` options when calling GRAFIMO from
# command-line, in order to do this
...
# VGs. GRAFIMO allows the user to study the recombinats that can be 
# obtained  from the genomic variants used for building the VG.
# To do so, it is sufficient to add the `--recomb` option
